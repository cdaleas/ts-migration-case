...
import {
  ...
  getCustomerName,
  getFirstAdminStaticToken,
  instantiateKnexInstanceFromValues,
} from './utils/config';
...
export default defineHook(({ action }, hookExtensionContext) => {
  ...
  database = database || instantiateKnexInstanceFromValues(logger);
  ...
  action('server.start', (): void => {
    const handler = async () => {
      ...
      if (migrations && migrations.length > 0) {
        const headers = await getFirstAdminStaticToken(database, logger);
        if (!headers) {
          logger.error(
            `No admin static token found migrations will not be run !`
          );
          return;
        }
